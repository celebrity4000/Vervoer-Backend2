        ...
        if (this.spacesList) {
          this.spacesList.forEach((value, key: string) => {
            for (let i = 1; i <= value.count; i++) {
              result.add(
                generateParkingSpaceID(key, i.toString())
              );
            }
          });
        }
...
export interface IGarageBooking {
...
export const GarageBooking = mongoose.model(
  "GarageBooking",
  garageBookingSchema
);