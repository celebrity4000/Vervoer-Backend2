...
export class BookingNotificationManager {
  ...
  static async notifyUserOfDriverResponse(
    bookingId: string,
    ...
    try {
      ...
      if (response === "accepted") {
        const title = booking.isScheduled
    ...
    try {
      ...
      for (const booking of upcomingBookings) {
        ...
        const timeLeft = Math.round(
          (booking.scheduledPickupDateTime!.getTime() - new Date().getTime()) /
            (1000 * 60),
        );
  ...
  static async suggestAlternativeDrivers(
